...
public class level3 : MonoBehaviour
{
    ...
    // The number of followers to spawn
    public int numFollowers = 2;

    // A list to store the spawned followers
    ...
    void Update()
    {
        foreach (GameObject spawnedFollower in spawnedFollowers)
        {
            Vector3 direction = (player.position - spawnedFollower.transform.position).normalized;
            spawnedFollower.transform.position += direction * followSpeed * Time.deltaTime;
        }
    }
    ...
    private void SpawnFollowers(int amount)
    {
        for (int i = 0; i < amount; i++)
        {
            ...
            int x = Random.Range(0, width);
            int z = Random.Range(0, height);
            Vector3 position = vertices[z * width + x];

            // Instantiate the follower prefab at the chosen position and add it to the list of spawned followers
            GameObject spawnedFollower = Instantiate(followerPrefab, position, Quaternion.identity);
